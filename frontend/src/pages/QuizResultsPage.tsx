...
const QuizResultsPage = () => {
  ...
  // If the user lands on this page directly without result data, redirect them home.
  if (!result) {
        ...
        <Accordion type="single" collapsible className="w-full">
          {result.results.map((res, index) => (
            <AccordionItem key={index} value={`item-${index}`}>
              <AccordionTrigger className={`flex justify-between items-center ${res.isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                <div className="flex items-center gap-2">
                    {res.isCorrect ? <CheckCircle className="h-5 w-5"/> : <XCircle className="h-5 w-5"/>}
                    <span className="text-left">{res.questionText}</span>
                </div>
              </AccordionTrigger>
